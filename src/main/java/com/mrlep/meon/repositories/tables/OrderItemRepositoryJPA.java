...
public interface OrderItemRepositoryJPA extends JpaRepository<OrderItemEntity, Integer> {
    ...
    @Query(value = "SELECT count(*) FROM ORDER_ITEM o WHERE o.is_active =1 AND o.bill_id = :billId AND o.status <> 3 AND o.status <> 5 AND o.status <> 6 ", nativeQuery = true)
    Integer countActiveOrderItemOfBill(Integer billId);

    @Query(value = "SELECT count(*) FROM ORDER_ITEM o WHERE o.is_active =1 AND o.bill_id = :billId AND o.status <> 5 AND o.status <> 6 ", nativeQuery = true)
    Integer countOrderItemOfBill(Integer billId);


    @Query(value = "UPDATE ORDER_ITEM set status =:status WHERE  bill_id = :billId AND status <> 5 AND status <> 6 ", nativeQuery = true)
