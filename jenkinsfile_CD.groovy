        ...
        if(deployer.contains(deployInput.submitter)){
            ...
            stage("5. ReTag version deploy"){
                withCredentials([usernamePassword(credentialsId: 'vinhtx3-habor', usernameVariable: 'username',
                    ...
                    """
                    sh """
                      docker --config ~/.docker/.vinhtx3 pull 10.60.156.72/telecare/covid-patient-f0:$env.GIT_TAG_DEPLOY
                      docker tag 10.60.156.72/telecare/covid-patient-f0:$env.GIT_TAG_DEPLOY 10.60.156.72/telecare/covid-patient-f0:$env.project_version
                      docker --config ~/.docker/.vinhtx3 push 10.60.156.72/telecare/covid-patient-f0:$env.project_version
                    """
                }
            }
            stage('6. Deploy to Productions'){
                echo "Version Deploy: $env.project_version"
                jenkinsfile_CI.release2k8s("k8s-vtnet","covid-patient-f0","telecare-k8s-config")
            }
...
def deployToPreRelease() {
    ...
    stage("4. ReTag version deploy"){
        withCredentials([usernamePassword(credentialsId: 'vinhtx3-habor', usernameVariable: 'username',
            ...
            """
            sh """
                docker --config ~/.docker/.vinhtx3 pull 10.60.156.72/telecare/covid-patient-f0:${versionRunningStaging}
                docker tag 10.60.156.72/telecare/covid-patient-f0:${versionRunningStaging} 10.60.156.72/telecare/covid-patient-f0:$env.project_version
                docker --config ~/.docker/.vinhtx3 push 10.60.156.72/telecare/covid-patient-f0:$env.project_version
            """
        }
    }
    stage('5. Deploy to Pre release'){
        ...
        if(env.preRelease == ""){
            ...
            release2k8s("${env.preRelease}","covid-patient-f0","telecare-k8s-config","telecare")
        }
    }
